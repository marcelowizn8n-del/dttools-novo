...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/addons/create-checkout-session", requireAuth, async (req, res) => {
    try {
      ...
      const addon = await storage.getAddon(addonKey);
      if (!addon) {
        return res.status(404).json({ error: "Addon not found" });
      }

      // Free addon doesn't need Stripe
      if (addon.price === 0) {
        const subscription = await storage.createUserAddonSubscription({
          userId: req.user.id,
          addonKey: addonKey,
          status: "active",
  ...
  app.post("/api/addons/cancel-subscription", requireAuth, async (req, res) => {
    try {
      ...
      // Cancel all related Stripe subscriptions at period end; webhook will update statuses
